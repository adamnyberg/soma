...
package DLX is
  ...
  function Is_Empty(Header : in Linked_Matrix) return Boolean;
  procedure Put_Row(Row : in Linked_Matrix; Amount : Integer);
  procedure Put(Header : in Linked_Matrix);
  ...
  function Has_A_Column_Without_Ones(Header: in Linked_Matrix) return Boolean;
  ...
  function Choose_Next_Row(Previous : in Linked_Matrix) return Linked_Matrix;
